...
class ImagesService:
    ...
    def get_image_ids(
        self, project_id: str, db: Session, offset=0, limit=sys.maxsize
    ) -> list[ImageIds]:
        statement = (
    ...
    def create_image_batch(
        self, imgs_dir: str, images: list[str], project_id: str, db: Session
    ) -> list[ImageBase]:
        ...
        for img_name in images:
            ...
            from_json_file = os.path.join(
                imgs_dir, f"{os.path.splitext(img_name)[0]}.json"
            )
            to_json_file = os.path.join(
                project_path, f"{os.path.splitext(img_name)[0]}.json"
            )
    ...
    def scan_folder(
        ...
        print("Track files")
        not_versioned_labels, updated_labels, deleted_images, new_images = (
            DataTracker.check_untracked(
                db_images=project_images, folder_path=project_path, db_labels=db_labels
            )
        )
        print("add labels")
